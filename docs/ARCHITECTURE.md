...
- **Data layer** – SQLAlchemy models in `models/database.py` represent seasons, games, practices, stat lines, skill entries, cached leaderboards, and supporting tables used throughout the app.【F:models/database.py†L6-L200】
...
| `admin/` | Admin blueprint with the bulk of data management views, imports, and leaderboard math. | Routes orchestrate CSV ingestion, practice parsing, roster tools, leaderboard computations, and cache management across many helpers.【F:admin/routes.py†L34-L200】 |
...
| `services/` | Background jobs, caching, external data fetchers. | Leaderboard cache utilities precompute payloads and manage freshness windows; NBA Summer League scraper collects daily box scores for alumni.【F:services/cache_leaderboard.py†L1-L200】【F:services/nba_stats.py†L11-L87】 |
...
| `tests/` | Extensive pytest suite. | Covers leaderboard formatting, permissions, practice parsing, caching, recruits flows, etc., providing living documentation for expected behaviors.【F:tests/test_leaderboard_cache.py†L1-L61】 |
...
- **Cached leaderboards** – `CachedLeaderboard` stores serialized leaderboard payloads that the caching services build/refresh to avoid recomputing heavy aggregates on each request.【F:models/database.py†L10-L37】【F:services/cache_leaderboard.py†L1-L200】
...
## Tooling & Automation
- **CLI commands** – App factory registers EYBL ingest and draft stock import commands plus a `flask cache` group to rebuild leaderboard caches individually or in bulk.【F:app/__init__.py†L323-L362】
...
- **Testing** – Run `pytest -q`; tests span caching, practice parsers, roster tools, skill workflows, and more—reading them is the fastest way to learn business rules.【F:README.md†L22-L28】【F:tests/test_leaderboard_cache.py†L1-L61】
...
3. **Trace a leaderboard** – Start at a public leaderboard view, follow into admin helper functions, and examine cache services to see how data flows from SQLAlchemy to Jinja tables.【F:public/routes.py†L141-L180】【F:admin/routes.py†L44-L200】【F:services/cache_leaderboard.py†L1-L200】
...
5. **Use tests as documentation** – Pick a test module relevant to your feature area (e.g., `test_player_detail_label_filter.py`, `test_practice_home_label_filter.py`) to see expected edge cases and payload shapes.【F:tests/test_leaderboard_cache.py†L1-L61】
